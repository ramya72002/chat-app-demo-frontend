...
const handleSendMessage = (e)=>{
  ...
  {groupedMessages.map((group, groupIndex) => (
    <div key={groupIndex}>
      <p className="text-xs text-center text-gray-500 my-2">{moment(group.date).format('MMMM Do, YYYY')}</p>
      {group.messages.map((msg, index) => (
        <div
          key={index}
          className={`p-1 py-2 rounded w-fit max-w-[280px] md:max-w-sm lg:max-w-md ${user._id === msg?.msgByUserId ? 'ml-auto bg-teal-100' : 'bg-white'} mb-2`} // Added margin-bottom to create space between messages
        >
